...
## 速览
**TLDR**：本研究针对肾脏病理AI中染色变异导致的分布偏移和捷径学习问题，构建了多中心多染色数据集，并通过贝叶斯CNN与ViT模型、对抗惩罚及熵正则化探讨模型应对捷径的能力，发现熵正则化可抑制染色捷径且保持性能稳健。 \
**Motivation**：染色差异可能成为肾小球病理AI模型的捷径，需探索无标签缓解偏差的策略。 \
**Method**：研究采用多中心多染色数据集，建立贝叶斯双头模型，结合对抗惩罚与熵最大化正则方法评估。 \
**Result**：模型能轻易识别染色信息，但熵正则化能维持随机染色预测而不损失病变分类准确率和校准性。 \
**Conclusion**：熵正则化在无需染色情况下有效减少捷径风险，提升模型鲁棒性。
