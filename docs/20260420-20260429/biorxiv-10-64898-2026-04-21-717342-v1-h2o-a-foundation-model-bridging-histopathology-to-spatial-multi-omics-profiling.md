...
## 速览
**TLDR**：该研究提出跨模态AI框架H2O，通过对比学习整合Vision Transformer和语言模型，将常规H&E病理图像转化为空间转录组和蛋白质组数据，实现低成本高精度的分子层次组织表征，显著提升多组学预测与组织图谱构建效率。 \
**Motivation**：空间组学成本高且难以规模化，而常规H&E染色普遍但缺乏分子层面的信息。 \
**Method**：模型将Vision Transformer与大型语言模型结合，采用对比学习对齐组织形态与分子语义信息。 \
**Result**：在25种器官和癌症类型的训练中，H2O在多个癌症基准上显著优于现有方法，并能从病理图像复原关键信号通路。 \
**Conclusion**：H2O通过融合图像和语言模型成功实现从常规病理图像推断空间多组学数据，展现出强大的泛化与生物学解释能力。
